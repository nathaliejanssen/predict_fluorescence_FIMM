...
def create_unet_model(input_shape):
    ...
    c8 = up(c7, c2, 128)
    c9 = up(c8, c1, 64)
    out = Conv2D(4, 1, activation=None)(c9)

    model = Model(inputs=unet_input, outputs = out, name = 'U-net')
    return model

def custom_mse(y_true, y_pred):
    weights = tf.constant([0.75,1,0.5,0.1])
...
def main():
    ...
    model = create_unet_model(input_shape = (1024, 1024, num_of_slices))
    opt = tf.keras.optimizers.Adam(learning_rate=1e-05)

    model.compile(optimizer = opt, loss = custom_mse, metrics = ['mse', 'mae', 'accuracy'])
    model.summary()

    my_callbacks = [
        tf.keras.callbacks.ModelCheckpoint(filepath = '/data/njanssen/models/weighted_2/model.{epoch:02d}--{val_loss:.2f}.h5', 
                                           monitor = 'val_loss'),
        tf.keras.callbacks.TensorBoard(log_dir = '/data/njanssen/logs/weighted_2')
    ]
    ...
    hist_df.to_csv('weighted_2.csv')

    model.save("weighted_2.h5")
